...
data class ShowMovieDetailsViewModel @Inject constructor(
    ...
    fun fetchTrailerUrl(movieId: String) = viewModelScope.launch(Dispatchers.Main) {
    ...
    private fun mapResult(result: CompleteResult<String>): ShowMovieDetailsViewState {
        return when (result) {
            is Success -> state.copy(trailerUrl = result.data)
    ...
    private val getRecommendatonIntr = getRecommendatonInteractor.asResult()

    fun moviesRecommendation(movieName: String) = viewModelScope.launch(Dispatchers.Main) {
        val result = withContext(Dispatchers.IO) { getRecommendatonIntr.invoke(movieName) }
        state = mapRecResult(result)

    }

    private fun mapRecResult(result: CompleteResult<ArrayList<Movie>>): ShowMovieDetailsViewState {
        return when (result) {
            is Success -> state.copy(movieRecList = result.data, isLoading = false, viewError = null)
            is Loading<*> -> state.copy(isLoading = true)
            is Fail -> state.copy(
                    viewError = SingleEvent(ViewErrorController.mapThrowable(result.throwable)),
                    isLoading = false
